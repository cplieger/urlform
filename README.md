...
- `Classify(raw string) Form` — total classification: every input lands in exactly one class, never an error. Input gets the WHATWG preprocessing first (edges trimmed, embedded tab/newline removed — the same hardening CPython adopted for CVE-2022-0391); backslashes are canonicalized to slashes for the parse exactly where a browser reads them that way (special schemes and schemeless forms, ahead of the query) while `Trimmed` keeps the emit form.
- `Form` — the extracted facts: `Class`, `Trimmed` (preprocessed, emit-safe), `Host` (lowercased ASCII-only fold; non-ASCII homograph bytes survive unfolded for fail-closed gates), `Scheme`, `Port` (extracted, deliberately not range-checked), `HasBackslash`, `HasTabOrNewline` (a whitespace-smuggling attempt was removed), `HasUserInfo`, `HostUnrecoverable`.
- `Class` — `ClassEmpty`, `ClassMalformed`, `ClassAbsolute`, `ClassHiddenHost` (a scheme-bearing parse hiding host evidence; for the authority-carrying special schemes the browser's reading is recovered into the facts: `https:/host/x` and `https:host/x` expose `host`, while `host:443/x` and `https://:443/x` stay evidence-free like the browser's own reading), `ClassProtocolRelative` (`//host/x` and the ambiguous `///x`), `ClassSchemelessHost` (`host/x`, where a browser navigates to `host`), `ClassRelative` (`/x`).
...
- **WHATWG input preprocessing.** Browsers delete embedded tab/newline wherever they appear and trim C0-control/space edges before parsing, so a string-level gate that skips this reads a different URL than the reader's browser will. `Classify` runs both steps first; `HasTabOrNewline` records a removed smuggling attempt, and `Trimmed` is already clean to emit. Edge trimming is deliberately widened to all Unicode whitespace (an NBSP-wrapped link still classifies; over-trimming errs fail-safe).
...
- **Bounded and total.** Allocation is bounded and linear in the input: the trimmed copy, plus at most one same-sized copy each for tab/newline stripping and backslash canonicalization when those bytes are present. Unparseable input is a class (`ClassMalformed`), not an error. Fuzz targets pin the exactly-one-class, nil-parse-only-for-no-facts, tab/newline-stripping, and metamorphic backslash invariants; rapid properties cover the canonicalization and smuggling-insensitivity laws on every PR; and a WPT-derived fixture corpus (`testdata/whatwg-fixtures.json`) pins the covered divergence set against the browser's documented reading.
